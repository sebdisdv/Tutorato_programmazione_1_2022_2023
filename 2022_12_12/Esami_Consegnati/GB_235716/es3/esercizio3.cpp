...
list * estrai( list * dip, list *ent, list *usc ) {
    list * p = NULL;    
    ///scorro la lista dipendenti
    for (list * s = dip; s!=NULL; s = s->next ) {
        int dipendente = s->code;
        int conta_ent=0, conta_usc=0;
        ///scorro e conto la lista entrate
        for (list * j = ent; j!=NULL; j = j->next ) {
            if (j->code == dipendente) {
                conta_ent++;
            }
        }
        for (list * j = usc; j!=NULL; j = j->next ) {
            if (j->code == dipendente) {
                conta_usc++;
            }
        }
        if (conta_ent == conta_usc) {
            if (p != NULL) {
                list * q = p;
                while (q->next != NULL) {
                    q = q->next;
                }
                q->next= new list{dipendente, NULL};
            } else {
                p = new list{dipendente, NULL} ;
            }       
